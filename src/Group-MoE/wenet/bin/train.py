...
def get_args():
    ...
    parser.add_argument('--cv_datasets', required=True, action='append',help='cv data file')
...
def main():
    ...
    if distributed:
        logger.debug('training on multiple gpus, this gpu {}'.format(args.gpu))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # train_acc_asr = 0
        # log_line = ' '
        train_acc_asr, train_seen_utts, train_time, log_line = executor.train(model, optimizer, scheduler, train_data_loader, device,
        ...
        for cv_data_loader in cv_dataloaders:
            ...
            wer = (1 - att_acc_asr) * 100
            wer_asr_all.append(f'wer:{wer:.2f}%')
            cv_loss = total_loss / num_seen_utts
            cv_loss_all.append(cv_loss)
            logger.debug('Epoch {} CV info cv_loss {:.4f} att_acc_asr {:.4f}'.format(epoch,cv_loss,att_acc_asr))
        if args.rank == 0:
            ...
            save_checkpoint(
                model, save_model_path, {
                    ...
                    'lr': lr,
                    'cv_loss': cv_loss,
                    'cv_loss_all': cv_loss_all,
                    'step': executor.step,
                    'cv-utterance_nums': num_seen_utts,
                    'cv-att_asr_wer': wer_asr_all,
                    'train-utterance_nums': train_seen_utts*args.world_size,
                    'train-att_acc_asr': f'{train_acc_asr:.4f}',
                    'train-time': int(train_time),
                    'gpu_nums': args.world_size,
