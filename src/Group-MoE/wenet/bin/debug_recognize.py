...
def get_args():
    config_path = "exp/train.yaml"
    dict_path = "data/dict/mix_dict.txt"
    test_data = "data/data.list"
    checkpoint = "exp/init.pt"
    ctc_weight = 0.5
    result_file = "./exp/rec"
    parser = argparse.ArgumentParser(description='recognize with your model')
    parser.add_argument('--config', default=config_path, help='config file')
    parser.add_argument('--test_data', default=test_data, help='test data file')
    ...
    parser.add_argument('--checkpoint', default=checkpoint, help='checkpoint model')
    parser.add_argument('--dict', default=dict_path, help='dict file')
    ...
    parser.add_argument('--penalty',
                        ...
                        default=0.0,
                        help='length penalty')
    parser.add_argument('--result_file', default=result_file, help='asr result file')
...
def main():
    ...
    os.chdir(os.path.join(current_dir, 'Language-bias'))
    args = get_args()
    logger_rec = get_Mylogger("rec", "./exp")
        ...
        for batch_idx, batch in enumerate(test_data_loader):
            ...
            # with other batch decoding mode
                ...
                hyps = model.hlg_rescore(
                    ...
                    word=args.word,
                    symbol_table=symbol_table)
            logger_rec.info(f"hyps_id:{hyps}")
            logger_rec.info(f"target_id:{target}")
            for i, key in enumerate(keys):
                ...
                logger_rec.info('{} {}'.format(key, args.connect_symbol.join(content)))
