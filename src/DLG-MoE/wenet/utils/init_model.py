...
from wenet.transformer.encoder import ConformerEncoder, TransformerEncoder, ConformerMoE
...
def init_model(configs):
    ...
    if encoder_type == 'conformer_moe':
        moe_conf = configs['moe_conf']
        if moe_conf['lid_route']:
            frame_lid = FrameLID(4, configs['encoder_conf']['output_size'])
        else:
            frame_lid = None
    # inter_ctc = ConditionalCTC(vocab_size, configs['encoder_conf']['output_size'])
    inter_idx = configs['inter_idx']
    if len(inter_idx) > 0:
        interctc = CTC(vocab_size, configs['encoder_conf']['output_size'])
    else:
        interctc = None
        inter_idx = None
    # inter_ctc = ConditionalCTC(vocab_size, ctc.ctc_lo, configs['encoder_conf']['output_size'])
    if encoder_type == 'conformer':
        encoder = ConformerEncoder(input_dim,
                                   global_cmvn=global_cmvn,
                                   intermediate_layers=inter_idx,
                                   **configs['encoder_conf'])
    elif encoder_type == 'conformer_moe':
        encoder = ConformerMoE(input_dim,
                               moe_conf=moe_conf,
                               global_cmvn=global_cmvn,
                               intermediate_layers=inter_idx,
                               frame_lid=frame_lid,
    ...
    logger_model.info(f"encoder_params:{encoder_params}")
    logger_model.info(f"decoder_params:{decoder_params}")
