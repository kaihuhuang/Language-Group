...
def main():
    args = get_args()
    logger_rec = get_Mylogger(args.mode,os.path.dirname(args.result_file))
    logger_rec.info(args)
    ...
    with torch.no_grad(), open(args.result_file, 'w') as fout:
        ...
        for batch_idx, batch in enumerate(test_data_loader):
            ...
            for i, key in enumerate(keys):
                ...
                logger_rec.info('{} {}'.format(key, args.connect_symbol
                                            .join(content)))
                fout.write('{} {}\n'.format(key, args.connect_symbol
                                            .join(content)))
        end_time = time.time()
        elapsed_time = (end_time - start_time)
        logger_rec.info(f"共耗时{elapsed_time}s")
