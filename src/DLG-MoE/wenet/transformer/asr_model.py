...
from wenet.transformer.global_router import FrameLID
...
class ASRModel(torch.nn.Module):
    ...
    def forward(
    ...
    ) -> Tuple[Dict[str, Optional[torch.Tensor]], int, int, str]:
        ...
        #  For convenience, we directly obtain the token-level LID groud-truth based on the IDs from the vocabulary table. 
        #  In fact, it would be better to load this in the dataloader.
        masked_LID = text.clone()
        masked_LID[(masked_LID <= 10)] = 0
        masked_LID[(masked_LID > 10) & (masked_LID < 5008)] = 2
        masked_LID[masked_LID > 5007] = 1
        # 1. Encoder
        encoder_out, encoder_mask, inter_outputs = self.encoder(speech, speech_lengths)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
        if self.frame_lid is None:
            frame_lid_loss = torch.tensor(0.0, device=encoder_out.device)
        else:
            frame_lid_loss = self.frame_lid(inter_outputs[5], encoder_out_lens, masked_LID, text_lengths)
        # 2a. Attention-decoder branch
        if self.ctc_weight != 1.0:
            loss_att, right_token, all_token = self._calc_att_loss(encoder_out, encoder_mask, text, text_lengths)
        ...
        loss_inter_ctc = torch.tensor(0.0, device=encoder_out.device)
        if self.inter_idx is not None:
            for index in self.inter_idx:
                loss_inter_ctc += self.interctc(inter_outputs[index], encoder_out_lens, text, text_lengths)
            loss_inter_ctc /= len(self.inter_idx)
        else:
            loss_inter_ctc = loss_inter_ctc
        loss_inter = loss_inter_ctc + frame_lid_loss
        ...
        if loss_ctc is None:
            ...
            loss = self.ctc_weight * loss_ctc + (1 - self.ctc_weight) * loss_att + 0.1 * loss_inter
        loss_dict = {}
        loss_dict = {"loss": loss,  "loss_att": loss_att, "loss_ctc": loss_ctc, "frame_lid_loss": frame_lid_loss, "loss_inter_ctc": loss_inter_ctc}
        log_line = f"loss={self.ctc_weight}*loss_ctc+{1 -self.ctc_weight}*loss_att+0.1*frame_lid_loss+0.1*loss_inter_ctc"
        return loss_dict, int(right_token), int(all_token), log_line
    ...
    def _calc_att_loss(
        ...
        numerator, denominator = th_accuracy(
            decoder_out.view(-1, self.vocab_size),
            ys_out_pad,
            ignore_label=self.ignore_id,
        )
        return loss_att, numerator, denominator
